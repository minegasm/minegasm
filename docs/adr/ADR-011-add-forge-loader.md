...
types. A matching `META-INF/mods.toml` (real Forge's file name and schema, `mandatory=true` rather than
...
is resolved, either via an upstream fix in `architectury-loom`/`gg.meza.stonecraft`, or a different approach
