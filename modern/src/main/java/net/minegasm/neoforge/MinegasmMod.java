...
/**
 * Client-only mod entrypoint for the NeoForge loader (brief §5.1, ADR-002/003). Owns the
 * loader-independent {@link MinegasmClient} and drives it from the client tick: each tick it samples
 * state and forwards discrete events, then hands the snapshot to the engine. Registers key bindings
 * (panic stop, quick connect) and the in-game config screen. No device I/O happens on the client
 * thread.
 *
 * <p>Lives in the shared root {@code src} behind a whole-file {@code //? if neoforge} Stonecutter
 * loader guard: for any non-NeoForge variant the guard comments the entire file out, so its
 * {@code net.neoforged.*} imports never reach a Fabric or Forge compile (docs/adr/ADR-013). The one
 * vanilla API that differs between the 26.1.2 and 26.2 lines (the toast-manager accessor) is handled
 * by {@link McCompat}, so this single file serves every Minecraft line rather than one copy per line.
 */
    ...
    *///?}
    ...
    private void sendMode(CommandSourceStack source) {
        source.sendSuccess(() -> Component.translatable("minegasm.command.mode_current",
                client.config().raw().profile().mode().name().toLowerCase(Locale.ROOT)), false);
    }
    ...
    private void sendRecipe(CommandSourceStack source) {
        source.sendSuccess(() -> Component.translatable("minegasm.command.recipe_current",
                client.config().raw().profile().recipePack()), false);
    }
    ...
    private int recipeFromCommand(CommandSourceStack source, String name) {
        ...
        source.sendSuccess(() -> Component.translatable("minegasm.command.recipe_set", selected), false);
        return 1;
    }
    ...
    private void sendBridge(CommandSourceStack source) {
        source.sendSuccess(() -> Component.translatable("minegasm.command.bridge_current",
                client.config().raw().bridges().get(0).enabled() ? "on" : "off"), false);
    }
    ...
    private int bridgeFromCommand(CommandSourceStack source, boolean enable) {
        ...
        source.sendSuccess(() -> Component.translatable("minegasm.command.bridge_set",
                enable ? "on" : "off"), false);
        return 1;
    }

    private void sendAdapter(CommandSourceStack source) {
        source.sendSuccess(() -> Component.translatable("minegasm.command.adapter_current",
