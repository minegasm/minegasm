...
public final class AccumulationProcessor {
    ...
    public void contribute(AccumulationParams params, GameEventKind kind, boolean ore, float strength) {
        ...
        switch (kind) {
            case ATTACK:
                delta = params.contribution("attack");
                break;
            case HURT:
            case EXPLOSION:
                delta = params.contribution("hurt");
                break;
            case BLOCK_BROKEN:
                delta = ore ? params.contribution("oreBreak") : params.contribution("blockBreak");
                break;
            case PLACE:
                delta = params.contribution("place");
                break;
            case XP_GAIN:
                delta = params.contribution("xpPerFive") * (0.5 + 4.5 * HapticMath.clamp01(strength));
                break;
            case ADVANCEMENT:
                delta = params.contribution("advancement");
                break;
            case HARVEST:
                delta = params.contribution("harvest");
                break;
            case FISHING_BITE:
                delta = params.contribution("fishing");
                break;
            case MINING_ACTIVE:
            case VITALITY:
            case AMBIENT:
    ...
    public float level(AccumulationParams params) {
        ...
        switch (params.outputCurve()) {
            case "smoothstep":
                return HapticMath.smoothstep(ratio);
            case "linear":
                return ratio;
            case "square":
                return ratio * ratio;
