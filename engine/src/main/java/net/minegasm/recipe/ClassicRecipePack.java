...
public final class ClassicRecipePack implements RecipePack {
    ...
    public Optional<HapticScene> resolve(RecipeContext ctx) {
        switch (ctx.intent().kind()) {
            case ATTACK:
                return plateau(ctx, 3000, 1000, true, HapticRole.IMPACT);
            case HURT:
                return plateau(ctx, 3000, 1000, true, HapticRole.IMPACT);
            // Legacy "mine" fired on the completed break; ore blocks add the boost window.
            case BLOCK_BROKEN:
                return plateau(ctx, 3000, ctx.intent().hasTag("ore") ? 1000 : 0,
                        ctx.intent().hasTag("ore"), HapticRole.REWARD);
            // Legacy had no continuous active-mining texture; suppress it in Classic.
            case MINING_ACTIVE:
                return Optional.empty();
            case PLACE:
                return plateau(ctx, 3000, 0, false, HapticRole.IMPACT);
            case XP_GAIN:
                return plateau(ctx, xpDurationMs(ctx), 1000, true, HapticRole.REWARD);
            case ADVANCEMENT:
                return plateau(ctx, advancementDurationMs(ctx), 1500, true, HapticRole.REWARD);
            case FISHING_BITE:
                return plateau(ctx, 1500, 0, false, HapticRole.WARNING);
            case HARVEST:
                return plateau(ctx, 150, 0, false, HapticRole.REWARD);
            case VITALITY:
                return plateau(ctx, 3000, 3000, true, HapticRole.WARNING);
            case EXPLOSION:
                return plateau(ctx, 500, 0, false, HapticRole.IMPACT);
            case AMBIENT:
                return Optional.empty();
            default:
