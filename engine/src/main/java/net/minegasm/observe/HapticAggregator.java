...
public final class HapticAggregator {
    ...
    private java.util.Optional<HapticIntent> mapDiscrete(RawGameEvent e, long nowNs) {
        switch (e.kind()) {
            case ATTACK: {
                Set<String> tags = e.getBool("critical", false)
                        ? java.util.Collections.singleton("critical") : java.util.Collections.emptySet();
                float strength = e.getFloat("cooldown", 0.6f);
                return java.util.Optional.of(intent(GameEventKind.ATTACK, strength, MaterialFeel.UNKNOWN,
            ...
            case BLOCK_BROKEN: {
                ...
                return java.util.Optional.of(intent(GameEventKind.BLOCK_BROKEN, hardness,
                        material(e), SpatialDirection.NONE, tags, e.gameTick(), nowNs));
            }
            case PLACE:
                return java.util.Optional.of(intent(GameEventKind.PLACE, e.getFloat("strength", 0.4f),
                        material(e), SpatialDirection.NONE, java.util.Collections.emptySet(),
                        e.gameTick(), nowNs));
            case HARVEST:
                return java.util.Optional.of(intent(GameEventKind.HARVEST, e.getFloat("strength", 0.5f),
                        material(e), SpatialDirection.NONE, java.util.Collections.emptySet(),
                        e.gameTick(), nowNs));
            case FISHING_BITE: {
                if (nowNs - lastFishingBiteNs < FISHING_REFRACTORY_NS) {
                    return java.util.Optional.empty(); // one pulse per bite
                }
                lastFishingBiteNs = nowNs;
                return java.util.Optional.of(intent(GameEventKind.FISHING_BITE, 0.7f, MaterialFeel.UNKNOWN,
                        SpatialDirection.NONE, java.util.Collections.emptySet(), e.gameTick(), nowNs));
            }
            case ADVANCEMENT: {
                String frame = e.get("frame", String.class).orElse("task").toLowerCase(java.util.Locale.ROOT);
                return java.util.Optional.of(intent(GameEventKind.ADVANCEMENT, 0.75f, MaterialFeel.UNKNOWN,
                        SpatialDirection.NONE, java.util.Collections.singleton(frame), e.gameTick(), nowNs));
            }
            case EXPLOSION: {
                ...
                return java.util.Optional.of(intent(GameEventKind.EXPLOSION, strength, MaterialFeel.UNKNOWN,
                        SpatialDirection.OMNI, java.util.Collections.emptySet(), e.gameTick(), nowNs));
            }
            default:
    ...
    private java.util.Optional<HapticIntent> coalesceXp(StateTransitions t, ClientStateSnapshot snapshot,
        ...
        Set<String> tags = pendingXpLeveled ? java.util.Collections.singleton("levelup") : java.util.Collections.emptySet();
    ...
    private HapticIntent mining(ClientStateSnapshot s, long nowNs) {
        Set<String> tags = s.miningTarget().map(pos -> java.util.Collections.singleton("pos:" + pos)).orElse(java.util.Collections.emptySet());
    ...
    private java.util.Optional<HapticIntent> vitality(StateTransitions t, RuntimeConfig config, long nowNs) {
        ...
        Set<String> tags = critical ? java.util.Collections.singleton("critical") : java.util.Collections.emptySet();
