...
public final class PrimitiveEvaluator {
    ...
    public static float levelAt(HapticPrimitive primitive, long elapsedNs) {
        ...
        // The trailing throw keeps the exhaustiveness a switch would give: a new HapticPrimitive
        // subtype fails loudly here instead of silently returning 0.
        if (primitive instanceof HapticPrimitive.Impulse) {
            return impulse((HapticPrimitive.Impulse) primitive, ms);
        } else if (primitive instanceof HapticPrimitive.Texture) {
            return texture((HapticPrimitive.Texture) primitive, ms);
        } else if (primitive instanceof HapticPrimitive.Rumble) {
            return rumble((HapticPrimitive.Rumble) primitive, ms);
        } else if (primitive instanceof HapticPrimitive.Sweep) {
            return sweep((HapticPrimitive.Sweep) primitive, ms);
        } else if (primitive instanceof HapticPrimitive.BeatPattern) {
    ...
    private static float sweep(HapticPrimitive.Sweep s, double ms) {
        ...
        switch (s.easing()) {
            ...
            case EASE_IN_OUT:
                ...
                break;
            default:
                throw new IllegalStateException("Unknown Easing: " + s.easing());
        }
