 ...
 * Minecraft 1.12.2. Mirrors the modern legacy-import screen; the legacy file is never modified.
 */
public final class ClassicLegacyImportScreen extends GuiScreen {
    ...
    public void initGui() {
        ...
        int cx = width / 2;
        int y = height - 52;
        addButton(new GuiButton(ID_APPLY, cx - 102, y, 100, 20, "Apply"));
        addButton(new GuiButton(ID_CANCEL, cx + 2, y, 100, 20, "Cancel"));
    }
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        drawCenteredString(fontRenderer, "Import legacy Minegasm config", width / 2, 18, 0xFFFFFF);
        drawCenteredString(fontRenderer, "From minegasm-client.toml", width / 2, 34, 0xA0A0A0);
        int y = 52;
        int limit = Math.min(rows.size(), Math.max(0, (height - 116) / 10));
        for (int i = 0; i < limit; i++) {
            drawCenteredString(fontRenderer, rows.get(i), width / 2, y + i * 10, 0xD0D0D0);
        }
        if (rows.size() > limit) {
            drawCenteredString(fontRenderer, "... and " + (rows.size() - limit) + " more",
                    width / 2, y + limit * 10, 0xA0A0A0);
        }
        drawCenteredString(fontRenderer, "The legacy file is left unchanged.",
