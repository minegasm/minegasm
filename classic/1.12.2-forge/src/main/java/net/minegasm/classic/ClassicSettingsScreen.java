...
/**
 * The gameplay/connection settings sub-screen for Minecraft 1.12.2, reached from the dashboard's Settings
...
public final class ClassicSettingsScreen extends GuiScreen {
    ...
    public void initGui() {
        ...
        recipeBtn = addButton(new GuiButton(ID_RECIPE, lx, y0 + 2 * dy, 150, 20, recipeLabel()));
        modeBtn = addButton(new GuiButton(ID_MODE, lx, y0 + 3 * dy, 150, 20, modeLabel()));
        fatigueBtn = addButton(new GuiButton(ID_FATIGUE, lx, y0 + 4 * dy, 150, 20, fatigueLabel()));
        pauseBtn = addButton(new GuiButton(ID_PAUSE, lx, y0 + 5 * dy, 150, 20, pauseLabel()));
        stopUnloadBtn = addButton(new GuiButton(ID_STOPUNLOAD, lx, y0 + 6 * dy, 150, 20, stopUnloadLabel()));
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        int lx = width / 2 - 155;
        int rx = width / 2 + 5;
        drawCenteredString(fontRenderer, "Minegasm settings", width / 2, 8, 0xFFFFFF);
