...
/**
 * 1.7.10 mirror of the modern {@code MinegasmDeviceEditorScreen}: per-device enable/intensity cap/
 ...
 * {@link CustomizationScreen}'s class doc for why every button is a {@link CallbackButton}, why sliders
 * are flushed rather than callback-driven, and the 1.7.10 {@code fontRendererObj}/{@code buttonList}/
 * no-{@code IOException} API notes.
 */
public final class DeviceEditorScreen extends GuiScreen {
    ...
    private List<Row> buildRows() {
        ...
        for (DeviceEditorModel.DeviceRow device : deviceRows) {
            ...
            list.add(new Row("Enable", (x, y, w, h) -> {
                int toggleWidth = 50;
                buttonList.add(toggle(x, y, toggleWidth, h, () -> device.enabled, v -> {
                    device.enabled = v;
            ...
            for (net.minegasm.core.BodyRegion choice : DeviceEditorModel.regionChoices()) {
                ...
                list.add(new Row(firstRegion ? "Body region" : "", (x, y, w, h) ->
                        buttonList.add(new CallbackButton(x, y, w, h, label, () -> {
            ...
            for (DeviceEditorModel.FeatureRow feature : device.features) {
                ...
                list.add(new Row(featureLabel, (x, y, w, h) -> {
                    int toggleWidth = 50;
                    buttonList.add(toggle(x, y, toggleWidth, h, () -> feature.enabled, v -> {
                        feature.enabled = v;
                        device.deviceTouched = true;
                    }));
            ...
            if (device.calibrationApplies) {
                ...
                list.add(new Row("Calibration enabled", (x, y, w, h) -> buttonList.add(toggle(x, y, w, h,
                ...
                list.add(new Row("Invert", (x, y, w, h) -> buttonList.add(toggle(x, y, w, h,
                ...
                list.add(new Row("Require return to neutral", (x, y, w, h) -> buttonList.add(toggle(
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        drawCenteredString(fontRendererObj, "Minegasm Device Editor", width / 2, 16, 0xFFFFFF);
